	...
	return (
		<div>
			<div className='flex justify-around items-center'>
				<p className='font-black text-4xl lg:text-5xl m-[30px]'>
					Теплоизоляция
				</p>
			...
			>
				...
				<Input
					value={Addprase}
					onChange={e => setAddprase(e.target.value)}
				></Input>
			...
			<div className='lg:flex flex-wrap justify-around my-[50px]'>
				{data.map(el => {
					return (
						<div
							key={el.id}
							className='lg:w-[22%] w-[80%] mx-auto shadow-2xl text-center my-[30px]'
						>
							<Link to={`/info/${el.id}`}>
							...
							<div className='flex text-5xl justify-center gap-[20px] my-[20px] font-medium'>
								<button className='p-[10px] ' onClick={() => dispatch(dec(el))}>
									-
								</button>
								<p className='p-[10px] '>{el.count}</p>
								<button className='p-[10px] ' onClick={() => dispatch(inc(el))}>
									+
								</button>
